...
export default function TerapiaCapilarPage() {
  ...
  const rituals = data?.ritualsList || [];
  const vipServices = data?.vipServices || [];
  ...
  return (
    <div className="bg-cream min-h-screen pb-20">
        ...
        <div className="w-full md:w-1/2 bg-earth-brown flex flex-col justify-center px-10 md:px-20 order-1 md:order-2 text-sand-light">
          ...
          >
            <span className="text-olive-green text-xs uppercase tracking-[0.4em] font-bold mb-4 block">
              {data?.badge || "High Ticket Service"}
            </span>
            ...
            <button
              ...
              className="flex items-center gap-3 bg-olive-green hover:bg-olive-green/90 text-cream px-8 py-3 uppercase tracking-widest text-xs font-bold transition-colors w-fit"
            >
              <MessageCircle size={16} />
              {data?.whatsappCta || "Reservar Diagnóstico vía WhatsApp"}
            </button>
          </motion.div>
        </div>
      </section>

      {/* 2. DIAGNÓSTICO VIP — MICROCÁMARA + ESCÁNER */}
      <section className="py-24 px-6 max-w-6xl mx-auto border-b border-earth-brown/10">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-20">
          {vipServices.map((svc, idx) => (
            <motion.article
              ...
              viewport={{ once: true }}
              className="bg-white border border-earth-brown/10 rounded-2xl p-10 shadow-sm hover:shadow-md transition-shadow"
            >
          ...
          {steps.map((step, idx) => (
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <p className="text-earth-brown/70 leading-relaxed font-light text-center">
                  {step.desc}
                </p>
              </div>
            </motion.div>
      ...
      <section className="py-24 px-6 max-w-5xl mx-auto">
        <div className="text-center mb-16">
          <span className="text-olive-green text-xs uppercase tracking-widest font-bold">
            {t?.cards?.therapySub || "Clínica Capilar"}
          </span>
          <h2 className="text-4xl font-serif text-earth-brown mt-2">
            {data?.ritualsTitle || "Rituales Capilares"}
          </h2>
          <p className="text-earth-brown/60 italic mt-2 max-w-xl mx-auto leading-relaxed">
            {data?.ritualsDesc}
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {rituals.map((ritual, idx) => (
            <motion.article
              ...
              transition={{ delay: idx * 0.15 }}
              viewport={{ once: true }}
              className="bg-white rounded-2xl p-8 border border-earth-brown/10 shadow-sm hover:shadow-md hover:border-olive-green/30 transition-all group"
            >
              ...
              <p className="text-earth-brown/70 leading-relaxed font-light text-sm mb-6">
                {ritual.desc}
              </p>
              <div className="flex items-center gap-2 text-olive-green text-xs font-bold uppercase tracking-widest">
                <Droplets size={12} />
                {ritual.duration}
              </div>
            </motion.article>
      ...
      {/* 4. SOLUCIONES TERAPÉUTICAS */}
      <section className="py-24 px-6 max-w-5xl mx-auto bg-earth-brown/5 rounded-3xl mx-6 md:mx-auto">
        <div className="text-center mb-16 px-6">
          <h2 className="text-4xl font-serif text-earth-brown">
        ...
        <div className="grid md:grid-cols-2 gap-10 max-w-4xl mx-auto px-6">
          {solutions.map((sol, idx) => (
            <motion.div
              key={idx}
              whileHover={{ y: -5 }}
              className="bg-white rounded-2xl p-8 border border-earth-brown/10 hover:border-olive-green transition-all"
            >
              <div className="flex justify-between items-start mb-6">
                <div className="p-3 bg-cream rounded-full shadow-sm">
                  {idx === 0 ? (
      ...
      <section className="py-20 px-6 text-center mt-16">
        <div className="max-w-2xl mx-auto">
          ...
          <button
            onClick={handleWhatsApp}
            className="inline-flex items-center gap-3 bg-olive-green text-white px-12 py-4 rounded-full uppercase tracking-widest text-sm font-bold shadow-xl hover:bg-earth-brown transition-colors"
          >
            <MessageCircle size={18} />
            {data?.ctaRecoveryBtn || "Reservar Diagnóstico"}
          </button>
        </div>
      </section>
    </div>
