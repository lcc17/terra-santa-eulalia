...
export default function Navbar() {
  ...
  const desktopLogoHeight = isScrolled ? "h-16 md:h-20" : "h-28 md:h-36";

  return (
    <nav
      className="fixed w-full z-50 top-0 left-0 font-serif"
    ...
    >
      ...
      >
        {/* Mobile Layout - El padding vertical (py) ahora se reduce al hacer scroll mediante transiciones fluidas */}
        <div
          className={`md:hidden relative w-full flex items-center justify-center px-6 transition-all duration-700 ${isScrolled ? "py-2" : "py-5"}`}
        >
        ...
        {/* Desktop Layout - El padding vertical (py) ahora se achica dinámicamente al hacer scroll */}
        <div
          className={`hidden md:block max-w-screen-2xl mx-auto w-full px-12 transition-all duration-700 ${isScrolled ? "py-4 lg:py-6" : "py-16"}`}
        >
          <div className="relative flex items-center justify-between">
            ...
            <div className="flex items-center gap-12 lg:gap-16">
              <Link
                href="/filosofia"
                className="hover:text-olive-green hover:-translate-y-0.5 transition-all duration-1000 ease-out py-2"
              >
      ...
      <AnimatePresence>
        {isSelectionHover && (
          <motion.div
            ...
            style={{
              boxShadow:
                "inset 0 2px 8px rgba(139, 115, 85, 0.04), 0 1px 0 rgba(255,255,255,0.5)",
            }}
          ...
          >
            <div className="max-w-screen-2xl mx-auto px-6 py-12 grid grid-cols-12 gap-10">
              <div className="col-span-3 border-r border-sand-light/30 pr-8">
                <h3 className="font-serif text-2xl text-earth-brown mb-4 italic">
                  ...
                  categories.map((cat, idx) => (
                    ...
                    >
                      <div className="h-full p-4 hover:bg-white/50 rounded-sm transition-all border border-transparent hover:border-sand-light/30">
                        <span className="text-earth-brown font-serif text-lg group-hover:text-olive-green transition-colors block mb-1">
                          {cat}
                        </span>
                        <span className="text-[10px] uppercase tracking-wider text-earth-brown/40 group-hover:text-earth-brown/60">
                          {t.domestic?.readMore || "Explorar"}
                        </span>
                      </div>
                    </Link>
                  ))
                ) : (
                  <p className="text-sm opacity-50 col-span-4">
                    {t?.ui?.loading || "Cargando..."}
                  </p>
                )}
              </div>
            </div>
